...
<%
  ...
  If NOT IsEmpty(submit) Then
    ...
    If submit = "Restore" Then
      ...
      Set recordsetList = connection.Execute("UPDATE posts SET deleted='N' WHERE seq = '"& seq &"';")
      submitMessage = "restore post success"
    ElseIf submit = "Delete" Then
      seq = Request.Form("seq")
      Set recordsetList = connection.Execute("UPDATE posts SET deleted='Y' WHERE seq = '"& seq &"';")
    ...
    ElseIf submit = "Update" Then
      ...
      If title <> "" And content <> "" And status <> "" Then
        seq = Request.Form("seq")
        Set recordsetList = connection.Execute("UPDATE posts SET title='"& title &"', content='"& content &"', status='"& status &"' WHERE seq = '"& seq &"';")
  ...
  ElseIf NOT IsEmpty(seq) Then
    ' select post
    connection.Open()
    Set recordsetSingle = connection.Execute("SELECT * FROM posts WHERE seq="&seq)
    Set selectedPost = new Post
  ...
  Dim myPost
  Set recordsetList = connection.Execute("SELECT * FROM posts WHERE deleted='N'")
  ...
  Set recordsetList = connection.Execute("SELECT * FROM posts WHERE deleted='Y'")
  Do While Not recordsetList.EOF
    Set myPost = New Post
    myPost.Seq = recordsetList.Fields("seq")
    myPost.Title = recordsetList.Fields("title")
...
<!--#include file="layouts/header.asp"-->
  ...
  <h1 class="uk-title">Post Update</h1>
